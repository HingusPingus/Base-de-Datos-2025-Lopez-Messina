...
create event rep on schedule every 1 day starts datetime "2025-05-23 23:59:00" ends current_date()+interval 3 month do
begin
	insert into reports 
    value(null, 
...
begin
update products join orderdetails on products.productcode=orderdetails.productcode set buyPrice=buyprice*1.05 where orderdetails not in(select orderdetails from orders where requiredDate<curdate()-interval 1 month);
end//
delimiter ;